...
uint16_t MCP23017PI::readGPIO() {
	return readRegisterWord(MCP23017PI_GPIOA);
}
...
void MCP23017PI::writeRegister(uint8_t regAddress, uint8_t data) {
	...
	if (returnValue != 0) {
		std::cout << "Something went wrong during writing. regAddress: " << std::hex << int(regAddress) << ", data: " << int(data) << std::endl;
	}
    bcm2835_i2c_end();
}
...
uint8_t MCP23017PI::readRegister(uint8_t regAddress) {
	...
	bcm2835_i2c_begin();
	bcm2835_i2c_setSlaveAddress(_i2cAddress);
	uint8_t returnValue = bcm2835_i2c_read_register_rs((char *)&regAddress, &buffer, 1);
	if (returnValue != 0) {
		std::cout << "Something went wrong during writing. regAddress: " << std::hex << int(regAddress) << std::endl;
	}
    bcm2835_i2c_end();
...
uint16_t MCP23017PI::readRegisterWord(uint8_t regAddress) {
	...
	bcm2835_i2c_begin();
	bcm2835_i2c_setSlaveAddress(_i2cAddress);
	uint8_t returnValue = bcm2835_i2c_read_register_rs((char *)&regAddress, buffer, 2);
	if (returnValue != 0) {
		std::cout << "Something went wrong during writing. regAddress: " << std::hex << int(regAddress) << std::endl;
	}
    bcm2835_i2c_end();
